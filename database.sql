Get all users from Chicago.

SELECT * FROM "bank"
WHERE "city" LIKE 'Chicago';

Get all users with usernames that contain the letter a.

SELECT * FROM "bank"
WHERE "username" LIKE '%A%'
OR "username" LIKE '%a%';

The bank is giving a new customer bonus! Update all records 
with an account balance of 0.00 and a transactions_attempted of 0. 
Give them a new account balance of 10.00.

SELECT * FROM "bank"
WHERE "account_balance" = 0.00
AND "transactions_attempted" = 0;

Select all users that have attempted 9 or more transactions.

SELECT * FROM "bank"
WHERE "transactions_attempted" >= 9;

Get the username and account balance of the 3 users with the 
highest balances, sort highest to lowest balance. NOTE: Research LIMIT

SELECT "username", "account_balance" FROM "bank"
ORDER BY "account_balance"
LIMIT 3;

Get the username and account balance of the 3 users with the 
lowest balances, sort lowest to highest balance.

SELECT "username", "account_balance" FROM "bank"
ORDER BY "account_balance" DESC
LIMIT 3;

Get all users with account balances that are more than $100.

SELECT "username" FROM "bank"
WHERE "account_balance" > 100.00;

Add a new record.

INSERT INTO "bank" ("username", "city", "transactions_completed", "transactions_attempted", "account_balance")
VALUES ('wile e. coyote', 'dallas', 100, 100, 1000006.00);

The bank is losing money in Miami and Phoenix and needs to 
unload low transaction customers: Delete users that reside in 
miami OR phoenix and have completed fewer than 5 transactions.

--check first
SELECT "username" FROM "bank"
WHERE "transactions_completed" < 5
AND "city" LIKE "miami" OR "phoenix";

--delete
DELETE FROM "bank"
WHERE "transactions_completed" < 5
AND "city" LIKE "miami" OR "phoenix";
